...
export default function ReactRouterLibrary() {
    ...
    // - 페이지가 새로고침되면 상태 유지가 안됨
    
    // <Link> :
    // - 사용자가 해당 웹 애플리케이션 내에서 다른 페이지로 이동할 수 있도록 하는 컴포넌트
    // - a 요소의 href 로 페이지 전환을 하게되면 새로운 페이지로 전향되는 반면, <Link> 는 새로고침을 하지 않고 URL 만 변경함
    ...
    const onButtonClick = () => {
        if (count === 10) navigator('/zustand');   // 버튼을 누르면 숫자가 증가하고, 10이 되고 11번째 때 zustand 로 이동한다
        setCount (count + 1);
    }
...
export function ReactRouterPage2() {
    ...
    // useSearchParams :
    // - 현재 URL 에 있는 쿼리 문자열을 읽을 때 사용하는 훅 함수
    // - 쿼리 문자열에 대한 배열을 반환
